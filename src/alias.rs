...
/// Convenience type suitable for Futures returned by by [`tower::Service`] implemnetations
pub type SvcBoxFut<R, E> =
    Pin<Box<dyn Future<Output = std::result::Result<R, E>> + Send + 'static>>;

/// An [`http_body::Body`] type thats suitable as a "catch-all" body type inside an application
